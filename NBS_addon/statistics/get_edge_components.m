...
%If size of component measured with intensity, create N x N matrix containing the test statistic for each edge minus 
% the test statistic threshold (primary threshold)
if Intensity
...
% TODO: I'm concerned by this logic. Here, the smallest "sz_links" can be is "1" 
% (similar to before, where the smallest "i" could be was "1"). 
% So i+1 inevitably boosted components in "cluster_stats_map" up to "2", 
% which is then decremented back to "1" in the following. Thus, 
% this does not seem to remove components of size 1 as the following asserts.
% Consider whether to return this or just orig map without "+1"
